...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCSprite * entrybackground = [CCSprite spriteWithFile:@"main_bg.png"];
        entrybackground.position =ccp(size.width/2, size.height/2);
        [self addChild:entrybackground];
        
        // Creating Menu Items
        // Normal images and selected are the same 
        CCMenuItemImage *menuItem1 =[CCMenuItemImage itemFromNormalImage:@"main_btn_play.png" selectedImage:@"main_btn_play.png" target:self selector:@selector(goToPlay)];
...
//        gameMenu.anchorPoint=CGPointZero;
        ...
        gameMenu.position=ccp(size.width/2,size.height/2 - (.368 *size.height/2)); 
