...
- (void)loadResources:(id<ResourceLoaderDelegate>)delegate
{
	...
	for (NSString *resource in self.resources) {
		NSBlockOperation *operation = [NSBlockOperation blockOperationWithBlock:^{
			...
			@try {
				_loadedResources++;
				float progress = (float)_loadedResources / [self.resources count];//([self.resources count]-(float)_loadedResources) / [self.resources count];	//because the fuse is burning , so the percentage decreases.
...
//CCAtlasSpriteManager *bg = [CCAtlasSpriteManager spriteManagerWithFile:@"x.png"]
